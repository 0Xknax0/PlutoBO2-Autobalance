...
updateplayertimes()
{
    ...
    for(;;)
    {
        ...
        {
            level.players[nexttoupdate] updateplayedtime();
            maps/mp/gametypes/_persistence::checkcontractexpirations(level.players[nexttoupdate]);
        }
        wait(1);
    }
}
...
updateplayedtime()
{
    ...
    {
        timealive = int(min(self.timeplayed["alive"], level.timeplayedcap));
        maps/mp/gametypes/_persistence::incrementcontracttimes(self, timealive);
        self addplayerstat("time_played_alive", timealive);
    }
    pixendevent();
    if(game["state"] == "postgame")
...
changeTeam(team)
{
    ...
    maps\mp\gametypes\_globallogic_ui::updateObjectiveText(self);
    maps\mp\gametypes\_spectating::setspectatepermissions(self);
